...
class ProcTHOREnvironment:
    """Wrapper for the ai2thor controller for ProcTHOR scenes providing additional functionality
    and bookkeeping.
    ...
    def __init__(self, all_metadata_available: bool = True, **kwargs):
        ...
        if "houses_dir" in kwargs:
            self.houses_dir = kwargs["houses_dir"]
    ...
    @property
    def top_down_frame(self) -> np.ndarray:
    ...
    def reset(
        ...
        if scene_name is not None and scene_name != self.scene_name:
            with open(os.path.join(self.houses_dir ,f'{scene_name}.json')) as json_file:
