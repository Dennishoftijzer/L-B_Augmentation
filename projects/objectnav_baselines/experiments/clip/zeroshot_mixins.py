...
from allenact_plugins.clip_plugin.clip_preprocessors_openworld import (
    ClipResNetPreprocessor,
    ClipTextPreprocessor,
    ClipResNetPreprocessor_LB_Augmented
)
...
@attr.s(kw_only=True)
class OpenworldClipResNetPreprocessGRUActorCriticMixin:
...
class LB_AugResNetPreprocessGRUActorCriticMixin(OpenworldClipResNetPreprocessGRUActorCriticMixin):
    ...
    def preprocessors(self) -> Sequence[Union[Preprocessor, Builder[Preprocessor]]]:
        ...
        assert (
            np.linalg.norm(
                np.array(rgb_sensor._norm_means)
                - np.array(ClipResNetPreprocessor_LB_Augmented.CLIP_RGB_MEANS)
            )
            < 1e-5
        )
        assert (
            np.linalg.norm(
                np.array(rgb_sensor._norm_sds)
                - np.array(ClipResNetPreprocessor_LB_Augmented.CLIP_RGB_STDS)
            )
            < 1e-5
        )
